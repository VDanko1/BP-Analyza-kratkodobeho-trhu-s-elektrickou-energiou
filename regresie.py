...
def korelacia():
    ...
    filtered_data = pd.merge(df_dam[['price']], df_idm[['priceWeightedAverage']], left_index=True, right_index=True, suffixes=('_DAM', '_IDM')).dropna()
...
def korelacia_denna():
    ...
    # Vytvorte grid scatter plot
    sns.set(style="whitegrid")
    g = sns.FacetGrid(merged_data, col="hour", height=4, aspect=1.5, col_wrap=4)
    g.map(sns.scatterplot, "price", "priceWeightedAveragepä", alpha=0.7)
    g.add_legend()
    plt.show()
    ...
    #  plt.title(f'Correlation graph for DAM price and IDM priceWeightedAverage - Hour {hour}')
    #  plt.xlabel('Price DAM €/MWh')
    ...
    # Upravený výpis rovnice pre lineárnu regresiu
    #for hour, fit in enumerate(fit_coefficients):
...
def korelacia_nedenna():
    ...
    print(f"Correlation between DAM price and IDM priceWeightedAverage (All Hours): {merged_data['price'].corr(merged_data['priceWeightedAverage'])}")
